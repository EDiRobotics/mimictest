...
from mimictest.Nets.FiLMCondition import FiLMCondition
...
class EfficientNet(nn.Module):
    def __init__(
        ...
        FiLM_cond_channel: Optional[int] = None,
    ) -> None:
        """
        ...
        FiLM_conds.append(FiLMCondition(FiLM_cond_channel, firstconv_output_channels))
        ...
        for cnf in inverted_residual_setting:
            ...
            for _ in range(cnf.num_layers):
                ...
                FiLM_stage.append(FiLMCondition(FiLM_cond_channel, block_cnf.out_channels))
                stage_block_id += 1

            layers.append(nn.Sequential(*stage))
            FiLM_conds.append(nn.Sequential(*FiLM_stage))
        ...
        FiLM_conds.append(FiLMCondition(FiLM_cond_channel, lastconv_output_channels))

        self.features = nn.Sequential(*layers)
        self.FiLM_conds = nn.Sequential(*FiLM_conds)
    ...
    def _forward_impl(self, x: Tensor, cond: Tensor) -> Tensor:
        for stage_idx, stage in enumerate(self.features):
            if stage_idx != 0 and stage_idx != len(self.features)-1:
                for layer_idx, layer in enumerate(stage):
                    x = layer(x)
                    x = self.FiLM_conds[stage_idx][layer_idx](x, cond)
            else:
                x = stage(x)
                x = self.FiLM_conds[stage_idx](x, cond)

        '''
        # Zhuoheng: we return the embedding
        x = self.avgpool(x)
        x = torch.flatten(x, 1)
        x = self.classifier(x)
        '''
...
if __name__ == "__main__": 
    model=efficientnet_v2_s(weights="DEFAULT", FiLM_cond_channel=512)
